class BairrosController < ApplicationController
  ...
  def get_bairros   
    bairros = Bairro.where(cidade_id: params[:id]).order('nome ASC')
    bairros_json = bairros.map {|item| {:id => item.id, :nome => item.nome}}
  ...
  private
    ...
    def bairro_params
      params.require(:bairro).permit(:nome, :latitude, :longitude, :cidade_id)
    end
end
